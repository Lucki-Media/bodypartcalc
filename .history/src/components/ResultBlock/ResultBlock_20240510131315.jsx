    ...
    fetch(`${process.env.REACT_APP_URL}` + "/wp-json/bmh-hair-modifier-graph/v1/data")
      ...
      .then(data => {
        ...
        hairZonesFromCookie.forEach(zone => {
          ...
          if (matchedZone) {
            // Convert severity values to lowercase for comparison
            const lowerCaseSeverity = zone.Severity.toLowerCase();
            const matchedSeverity = Object.keys(matchedZone).find(key => key.toLowerCase() === lowerCaseSeverity);
            if (matchedSeverity) {
              // Determine cost based on severity
              const cost = parseInt(matchedZone[matchedSeverity]);
