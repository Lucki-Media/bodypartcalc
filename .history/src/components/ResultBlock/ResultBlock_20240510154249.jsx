...
const ResultBlock = ({onNext}) => {
  ...
  useEffect(() => {
    fetch(`${process.env.REACT_APP_URL}` + "/wp-json/bmh-hair-modifier-graph/v1/data")
      ...
      .then(data => {
        let totalCost = 0;
        let norwoodStageValue = 0; // Initialize norwood stage value
        ...
        hairZonesFromCookie.forEach(zone => {
          ...
          if (matchedStage) {
            // Extract the severe value for the stage and add to norwoodStageValue
            norwoodStageValue += parseInt(matchedStage.severe);
          }
        });

        // Update state with total cost and norwood stage value
        setResultData(totalCost);
        setNorwoodStageValue(norwoodStageValue);
      })
