    ...
    fetch(`${process.env.REACT_APP_URL}` + "/wp-json/bmh-hair-modifier-graph/v1/data")
      ...
      .then(data => {
        ...
        hairZonesFromCookie.forEach(zone => {
          ...
          const matchedStagesZone = data.norwood_stages.find(norwood_stages => norwood_stages.zones === zone.Hair_Zone);

          console.log('matchedStagesZone');
          console.log(matchedStagesZone.zones);
          console.log(matchedStagesZone.Severity);
          ...
          if (matchedStagesZone) {
            // Determine cost based on severity
            let stageCost;
            switch (zone.Severity) {
              case 'Minimal':
                stageCost = parseInt(matchedStagesZone.mild);
                break;
              case 'Moderate':
                stageCost = parseInt(matchedStagesZone.moderate);
                break;
              case 'Severe':
                stageCost = parseInt(matchedStagesZone.severe);
                break;
              default:
                stageCost = 0;
            }

            console.log('totalStageCost');
            console.log(totalStageCost);

            

            // Add cost to totalCost
            totalStageCost += stageCost;
          }
      ...
      <div className={styles.result_module}>
        <div className="global_small_container">
          <div className={styles.result_content_block}>
            ...
            <p>According to our calculations, you are Stage __ on the Norwood Scale, and your Hair Restoration Procedure will need around {resultData} number of grafts to completely restore your hair.</p>
