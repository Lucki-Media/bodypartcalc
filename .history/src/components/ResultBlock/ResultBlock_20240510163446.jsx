      ...
      .then(data => {
        ...
        hairZonesFromCookie.forEach(zone => {
          ...
          if (matchedStage) {
            ...
            norwoodValues = matchedStage.nw_stage_value;
            // Check if the calculated stage range falls within the nw_stage_value range
           
          }
        });

        console.log('norwoodValues');
        console.log(norwoodValues);

        

    //     if (minStageValue >= parseInt(norwoodValues.nw_stage_value.split('-')[0]) &&
    //        maxStageValue <= parseInt(norwoodValues.nw_stage_value.split('-')[1])) {
