  ...
  useEffect(() => {
    fetch(`${process.env.REACT_APP_URL}` + "/wp-json/bmh-hair-modifier-graph/v1/data")
      ...
      .then(data => {
        ...
        // Find matching hair data from filtered data based on skin tone, hair type, and hair color
        const matchedHair = filteredHairData.find(hair => hair.skintone === skinToneFromCookie && hair.hairtype === hairTypeFromCookie && hair.haircolor === hairColorFromCookie);
  
        console.log('matchedHair');
        console.log(filteredHairData);
