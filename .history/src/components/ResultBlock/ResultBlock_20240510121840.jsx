...
const ResultBlock = ({onNext}) => {
  ...
  useEffect(() => {
    fetch(`${process.env.REACT_APP_URL}` + "/wp-json/bmh-hair-modifier-graph/v1/data")
      ...
      .then(data => {
        // Find matching values from API response based on cookies
        const matchedZone = data.zones.find(zone => zone.zones === hairZonesFromCookie);
        ...
        console.log('hairZonesFromCookie');
        console.log(hairZonesFromCookie);
        ...
        const resultValue = matchedZone ? matchedZone.mild : 0;
        const resultValue2 = matchedHair ? matchedHair.mutliplier : 1;
        const resultValue3 = resultValue * resultValue2; // Example calculation

        setResultData({ resultValue, resultValue2, resultValue3 });
      })
