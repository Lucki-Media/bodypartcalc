    ...
    fetch(`${process.env.REACT_APP_URL}` + "/wp-json/bmh-hair-modifier-graph/v1/data")
      ...
      .then(data => {
        ...
        hairZonesFromCookie.forEach(zone => {
          ...
          // If a matching zone is found, perform calculations
          if (matchedZone) {
            // Example calculation
            resultData[zone.Hair_Zone] = {
              resultValue: matchedZone.mild * someCalculationBasedOnSeverity(zone.Severity),
              resultValue2: someCalculationBasedOnSeverity(zone.Severity),
              resultValue3: matchedZone.mild * someCalculationBasedOnSeverity(zone.Severity) * someCalculationBasedOnSeverity(zone.Severity)
            };
          }
        });
  
        // Find matching hair data from API response
        const matchedHair = data.hair.find(hair => hair.skintone === skinToneFromCookie && hair.hairtype === hairTypeFromCookie && hair.haircolor === hairColorFromCookie);
  
        // If matching hair data is found, perform calculations
        if (matchedHair) {
          // Example calculation
          resultData.hair = {
            resultValue: someCalculationBasedOnHairType(matchedHair.hairtype),
            resultValue2: someCalculationBasedOnHairColor(matchedHair.haircolor),
            resultValue3: someCalculationBasedOnSkinTone(matchedHair.skintone)
          };
        }
