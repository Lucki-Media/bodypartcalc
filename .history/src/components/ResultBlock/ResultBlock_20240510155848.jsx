      ...
      .then(data => {
        ...
        hairZonesFromCookie.forEach(zone => {
          ...
          if (matchedStage) {
            ...
            // Split the severity range and add to norwoodStageValue
            const [min, max] = severityValue.split('-').map(val => parseInt(val));
            norwoodStageValue += (max - min + 1); // Add 1 to include both min and max values

            console.log('matchedStage');
            console.log(min );
            console.log(max );
            console.log(norwoodStageValue );
