...
const ResultBlock = ({onNext}) => {
  ...
  useEffect(() => {
    fetch(`${process.env.REACT_APP_URL}` + "/wp-json/bmh-hair-modifier-graph/v1/data")
      ...
      .then(data => {
        let totalCost = 0;
        let minStageValue = Infinity; // Initialize minimum stage value to a very large number
        ...
        hairZonesFromCookie.forEach(zone => {
          ...
          if (matchedStage) {
            ...
            minStageValue = Math.min(minStageValue, min);
            // Update maxStageValue if the current max is greater than the stored max
            maxStageValue = Math.max(maxStageValue, max);
          }
        });
        ...
        setStageRange(`${totalMinStageValue}-${totalMaxStageValue}`);
      })
      .catch(error => console.error('Error fetching data:', error));
  }, [hairZonesFromCookie, skinToneFromCookie, hairTypeFromCookie, hairColorFrom
